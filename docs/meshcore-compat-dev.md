...
| Config writes/admin | Compatibility-local stores exist for advert name, channel 0, auto-add config, default flood scope name/key, and device PIN. Factory reset clears only these MeshCore-local compatibility values and never clears native LICHEN identity, OSCORE/link/group/routing, or radio state. Other writes still return `RESP_ERR + ERR_UNSUPPORTED_CMD`, including raw/radio/path operations, remote admin, reboot, private-key, signing flows, and secret-bearing channel imports. | `lichen/tests/meshcore_adapter` unsupported command coverage; `lichen/tests/meshcore_gateway_adapter.settings_nvs`; `test/vectors/meshcore_app_compat.json`. | Physical board flash retention validation remains separate follow-up work. |
| Message ingress | `SEND_CHANNEL_TXT_MSG` for channel `0`, plain text, and valid UTF-8 maps to the shared app-interface text submit boundary. It returns success only after the submit provider succeeds; without a provider it returns `ERR_UNSUPPORTED_CMD`. Direct peer text treats the first 6 bytes as a MeshCore public-key prefix and submits only when that prefix maps to exactly one known LICHEN app-identity peer; the submitted event carries that peer IID via `has_to_iid`. No match or collision returns `ERR_NOT_FOUND`. | `lichen/tests/meshcore_adapter`, `lichen/tests/meshcore_gateway_adapter`, `test/vectors/meshcore_app_compat.json`. | Physical client observation remains part of the MeshCore app smoke validation path. |
...
| `FACTORY_RESET` | Clear MeshCore compatibility-local display/channel/auto-add/default-flood/PIN state only. Native LICHEN identity, security, routing, group, and radio state are not reset through the MeshCore compatibility surface. |

...
Environment (adjust the SDK path to your install):

```bash
export ZEPHYR_SDK_INSTALL_DIR=$HOME/.local/share/safe-agent/.../zephyr-sdk-0.16.8
...
**Method:** read-only watch of the T-Echo `if02` console, tallying
...
reroll its epoch). Script: `scratchpad/soak.py` in the session workspace.

...
|---------|------------------|-----------------|--------------------|-------|
| Display + buttons + GNSS | LilyGO T-Echo (`t_echo_nrf52840`) | nRF52840, SX1262, USB, BLE-class MCU, user button, GDEW0154M10 e-ink hardware, L76K GPS UART model, PCF8563 RTC. | Board DTS exists. LoRa is enabled by app overlays; GPS UART is modeled but disabled pending focused validation; display bus is present but the e-ink child node still needs a bring-up overlay. | LICHEN reference |
...
| Headless/module | RAK4631 (`rak4631_nrf52840`) with Muzi R1 Neo as the first custom sibling | nRF52840 + SX1262, USB/BLE-class local transport, comfortable memory, and minimal UI. R1 Neo adds project-owned board data. | RAK4631 uses upstream board support plus LICHEN overlays. R1 Neo DTS exists but radio reset, bootloader, GNSS, and battery paths need hardware validation. | LICHEN reference |
...
|-----------------------|--------------------|-------|---------|--------|--------------------|----------------|------------------|---------|-----------------------|-----------|
| LilyGO T-Echo | nRF52840, 256 KB RAM / 1 MB flash | SX1262 modeled; enabled by overlays | GDEW0154M10 e-ink hardware; SPI bus modeled, display child node not yet present | One user button, LEDs | L76K GPS UART modeled, disabled; PCF8563 RTC enabled | Battery path unknown in current DTS | Native USB; BLE local transport is the intended nRF52840 app-compat path | Internal flash partitions | First-party board under `lichen/boards/lilygo/t_echo`; puck and gateway configs exist | LICHEN reference for display+buttons+GNSS |
...
| TTGO LoRa32 / T-Beam v1 / Heltec V2 class | ESP32, older flash/RAM class | SX1276/78 | OLED on common variants | Buttons vary by board | T-Beam v1 class may have GNSS; board-specific validation required | Battery/PMIC varies | UART; ESP32 BLE path not selected for MVP | Internal flash | TTGO LoRa32 overlay exists; lilygo_tbeam bridge fragment retired (no canonical target, FAIL per project-LICHEN-2u26.5); classic ESP32 family listed in top-level targets | Partner-owned long tail after SX127x path is stable |
...
- Vendor variants in the same family as the references: Heltec V2/V3/T114,
  ...
  carriers, and region-specific radio modules.
...
- First-party board files:
  - `lichen/boards/lilygo/t_echo/t_echo_nrf52840.dts`

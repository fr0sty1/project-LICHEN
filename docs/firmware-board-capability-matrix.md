...
|---------|------------------|-----------------|--------------------|-------|
| Display + buttons + GNSS | LilyGO T-Echo (`t_echo_nrf52840`) | nRF52840, SX1262, USB, BLE-class MCU, user button, GDEW0154M10 e-ink hardware, L76K GPS UART and 1PPS wired to P1.04, PCF8563 RTC. | Manufacturer schematic, pin map, and factory firmware confirm PPS wiring. Board DTS models only GPS UART, which remains disabled pending focused validation; PPS timer capture is not modeled, and the e-ink child node still needs a bring-up overlay. | LICHEN reference |

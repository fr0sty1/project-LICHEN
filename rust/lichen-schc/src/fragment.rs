...
pub const DEFAULT_WINDOW_SIZE: usize = WINDOW_SIZE;
pub const MAX_WINDOW_SIZE: usize = WINDOW_SIZE;
pub const TILE_SIZE: usize = 187;
pub const MIC_LENGTH: usize = 4;
pub const MAX_TILES: usize = 126;
pub const MAX_PACKET_SIZE: usize = MAX_TILES * TILE_SIZE;
pub const DEFAULT_RECEIVER_LIMIT: usize = 1281;
pub const MAX_ACK_REQUESTS: u8 = 4;
const BITMAP_MASK: u64 = (1u64 << WINDOW_SIZE) - 1;
...
impl<'a> Fragment<'a> {
    ...
    /// Parse and unshift a fragment into caller-provided tile storage.
...
impl Ack {
    ...
    pub fn write_to(&self, out: &mut [u8]) -> Result<usize, FragmentError> {
        ...
        out[..needed].fill(0);
        out[0] = self.rule_id;
        set_bit(&mut out[1..needed], 0, self.window != 0);
        for position in 0..kept {
            set_bit(
                &mut out[1..needed],
                2 + position,
                self.bitmap & (1u64 << (62 - position)) != 0,
            );
        }

...
fn malformed_codec_inputs_are_rejected() {
    assert!(Fragment::from_bytes(&[0x78, 0x7c, 0], &mut [0u8; 1]).is_err());
    let regular_nonzero_padding = [0xff; TILE_SIZE + 2];
    assert!(Fragment::from_bytes(&regular_nonzero_padding, &mut [0u8; TILE_SIZE]).is_err());
...
fn missing_all0_still_requests_final_window_ack() {
    ...
    let fragment = Fragment::from_bytes(&wire[..length], &mut [0u8; TILE_SIZE]).unwrap();

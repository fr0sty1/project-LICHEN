...
use lichen_schc::fragment::{
    compute_mic, receiver_abort, sender_abort, Ack, Fragment, FragmentSender,
    ReceiverResponse, SenderOutput, SenderStatus, DEFAULT_WINDOW_SIZE, MAX_ACK_REQUESTS,
    MAX_PACKET_SIZE, RULE_ID_A_TO_B, TILE_SIZE,
};

const DEFAULT_RECEIVER_LIMIT: usize = MAX_PACKET_SIZE;
...
fn sender_receiver_literal_recovery() {
    ...
    packet[374] = 0xa5;
    let mut sender = FragmentSender::new(&packet, RULE_ID_A_TO_B, DEFAULT_RECEIVER_LIMIT).unwrap();
    sender.start().unwrap();
    ...
    assert_eq!(&wire[..length], &[RULE_ID_A_TO_B, 0x20, 0, 0, 0, 0, 0, 0, 0]);

    let mut output = sender.handle_ack(ack);
    sender.write_next(&mut output, &mut wire).unwrap().unwrap();
    assert_eq!(&wire[..2], &[RULE_ID_A_TO_B, 0x7a]);
    ...
    assert_eq!(&wire[..length], &[RULE_ID_A_TO_B, 0x00]);
...
fn duplicate_regular_must_match() {
    ...
    let sender = FragmentSender::new(&packet, RULE_ID_A_TO_B, DEFAULT_RECEIVER_LIMIT).unwrap();
...
fn retry_limits_emit_aborts() {
    ...
    let mut wire = [0u8; 3];
    let length = sender.write_next(&mut output, &mut wire).unwrap().unwrap();
    assert_eq!(&wire[..length], &[RULE_ID_A_TO_B, 0xfe]);
    assert_eq!(sender.status(), SenderStatus::Aborted);

    let mut storage = [0u8; DEFAULT_RECEIVER_LIMIT];
    let mut receiver = FragmentReceiver::new(&mut storage).unwrap();
    for _ in 0..MAX_ACK_REQUESTS {
        assert!(!receiver.receive_bytes(&[RULE_ID_A_TO_B, 0x00]).unwrap().aborted);
    }
    let result = receiver.receive_bytes(&[RULE_ID_A_TO_B, 0x00]).unwrap();
    assert!(result.aborted);
    let length = result.response.unwrap().write_to(&mut wire).unwrap();
    assert_eq!(&wire[..length], &[RULE_ID_A_TO_B, 0xff, 0xff]);
}

#[test]
fn malformed_codec_inputs_are_rejected() {
    let mut tile = [0; TILE_SIZE];
    assert!(Fragment::from_bytes(&[RULE_ID_A_TO_B, 0x7c, 0], &mut tile).is_err());
    let regular_nonzero_padding = [0xff; TILE_SIZE + 2];
    assert!(Fragment::from_bytes(&regular_nonzero_padding, &mut tile).is_err());
    assert!(Ack::from_bytes(&[RULE_ID_A_TO_B, 0x40, 0]).is_err());
    assert!(Ack::from_bytes_for(
        &[RULE_ID_A_TO_B, 0x38, 0, 0, 0, 0, 0, 0, 0],
...
fn sender_output_retries_after_small_buffer() {
    ...
    let mut sender = FragmentSender::new(&packet, RULE_ID_A_TO_B, DEFAULT_RECEIVER_LIMIT).unwrap();
    sender.start().unwrap();
    let mut output = sender.handle_ack(Ack::new(RULE_ID_A_TO_B, 0, 1, false));
    assert!(sender.write_next(&mut output, &mut [0u8; 1]).is_err());
    let length = sender.write_next(&mut output, &mut wire).unwrap().unwrap();
    assert_eq!(length, TILE_SIZE + 2);
    assert_eq!(&wire[..2], &[RULE_ID_A_TO_B, 0x7d]);
    assert!(sender.write_next(&mut output, &mut [0u8; 1]).is_err());
    let length = sender.write_next(&mut output, &mut wire).unwrap().unwrap();
    assert_eq!(&wire[..length], &[RULE_ID_A_TO_B, 0x00]);
...
fn terminal_sender_invalidates_queued_output() {
    ...
    let mut output = sender.handle_ack(Ack::new(RULE_ID_A_TO_B, 0, 1, false));
    let mut abort = [0u8; 3];
    let length = receiver_abort(RULE_ID_A_TO_B).write_to(&mut abort).unwrap();
    assert_eq!(
...
fn ack_request_after_completion_starts_empty_context() {
    ...
    let length = result.response.unwrap().write_to(&mut wire).unwrap();
    assert_eq!(length, 10);
    assert_eq!(&wire[..length], &[RULE_ID_A_TO_B, 0, 0, 0, 0, 0, 0, 0, 0, 0]);
}

#[test]
fn missing_all0_still_requests_final_window_ack() {
    let packet = vec![0u8; TILE_SIZE * 63 + 1];
    let mut sender = FragmentSender::new(&packet, RULE_ID_A_TO_B, packet.len()).unwrap();
    sender.start().unwrap();
    let mut output = sender.handle_ack(Ack::new(RULE_ID_A_TO_B, 0, u64::MAX << 1, false));
    let mut wire = [0u8; TILE_SIZE + 2];

    let length = sender.write_next(&mut output, &mut wire).unwrap().unwrap();
    assert_eq!(length, TILE_SIZE + 2);
    assert_eq!(&wire[..2], &[RULE_ID_A_TO_B, 0x00]);
    ...
    let length = sender.write_next(&mut output, &mut wire).unwrap().unwrap();
    assert_eq!(&wire[..length], &[RULE_ID_A_TO_B, 0x80]);
    assert_eq!(sender.write_next(&mut output, &mut wire).unwrap(), None);
}
...
fn released_receiver_accepts_fresh_fragments() {
    let first = [0xa5];
    let second = [0x5a];
    let first_sender = FragmentSender::new(&first, RULE_ID_A_TO_B, DEFAULT_RECEIVER_LIMIT).unwrap();
    let second_sender = FragmentSender::new(&second, RULE_ID_A_TO_B, DEFAULT_RECEIVER_LIMIT).unwrap();
    let mut storage = [0u8; DEFAULT_RECEIVER_LIMIT];
    let mut receiver = FragmentReceiver::new(&mut storage).unwrap();
    ...
    let partial_sender = FragmentSender::new(&partial, RULE_ID_A_TO_B, DEFAULT_RECEIVER_LIMIT).unwrap();
    assert_eq!(
        receiver

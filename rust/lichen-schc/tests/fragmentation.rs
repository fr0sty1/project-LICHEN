...
#[test]
fn all1_first_single_tile_and_conflicting_duplicate() {
    let packet = [0xa5];
    let sender = FragmentSender::new(&packet, 0x79, DEFAULT_RECEIVER_LIMIT).unwrap();
    let all1 = sender.get_fragment(0).unwrap();
    let mut storage = [0u8; DEFAULT_RECEIVER_LIMIT];
    let mut receiver = FragmentReceiver::new(&mut storage).unwrap();
    let result = receiver.receive(&all1);
    ...
    let mut bad = all1;
    bad.mic[0] ^= 1;
    assert!(!receiver.receive(&bad).aborted);
    assert!(receiver.receive(&all1).aborted);
}

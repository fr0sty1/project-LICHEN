...
/// TH_4 = H(CBOR(TH_3) || CBOR(plaintext) || CBOR(cred_r)) per RFC 9528 §3.5.
fn transcript_4(th_3: &[u8; 32], plaintext: &[u8], cred_r: &[u8]) -> Result<[u8; 32], EdhocError> {
    let mut buf = heapless::Vec::<u8, 1024>::new();
...
fn encode_credential<const N: usize>(buf: &mut heapless::Vec<u8, N>, pubkey: &[u8]) -> Result<(), EdhocError> {
    let mut cose_key = heapless::Vec::<u8, 64>::new();
    cose_key.push_err(0xa2)?; // map(2)
    cose_key.push_err(0x01)?; cose_key.push_err(0x01)?; // 1: 1 (kty = OKP)
    cose_key.push_err(0x20)?; cose_key.push_err(0x06)?; // -1: 6 (crv = Ed25519)
    cose_key.push_err(0x21)?; // -2: bstr(pubkey)
...
/// Pending Message 2 from the responder, awaiting credential verification.
#[derive(Clone, Debug)]
...
impl PendingMessage2 {
    /// Return the ID_CRED extracted from Message 2.
    pub fn id_cred(&self) -> &IdCred {
        &self.id_cred
    }
}

/// Pending Message 3 from the initiator, awaiting credential verification.
#[derive(Clone, Debug)]
...
impl PendingMessage3 {
    /// Return the ID_CRED extracted from Message 3.
    pub fn id_cred(&self) -> &IdCred {
        &self.id_cred
    }
}
...
impl EdhocResponder {
    ...
    /// Create a new EDHOC responder from a generic connection identifier.
    pub fn new_with_rng<R: RngCore + CryptoRng, C: Into<ConnectionId>>(
        seed: [u8; 32],
        c_r: C,
        rng: &mut R,
    ...
    /// Create a new EDHOC responder.
    #[cfg(feature = "std")]
    pub fn new_from_std<C: Into<ConnectionId>>(
        seed: [u8; 32],
        c_r: C,
    ) -> Result<Self, OscoreError> {

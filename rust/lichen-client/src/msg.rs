...
//!
//! - `POST /msg/sent` request  : `{to: tstr, body: tstr, ack: bool}`
...
/// A message to send to a peer - the body of a `POST /msg/sent` request.
///
...
/// A sent-message record - the `POST /msg/sent` response and each element of
/// `GET /msg/sent`.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]

...
impl MeshtasticBridge {
    ...
    pub fn process_incoming(&mut self, packet: &MeshPacket) -> Result<IncomingResult, BridgeError> {
        ...
        match portnum {
            PortNum::IpTunnelApp => {
                // Raw IPv6 packet encapsulated in Meshtastic IP Tunnel App (PortNum 0x80).
                // Version check (first nibble==6) + extract src/dst from IPv6 header
                // (bytes 8-23 src, 24-39 dst). Converged from worker5/8/20/23/24 patterns.

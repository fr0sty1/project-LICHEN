...
fn identity(byte: u8) -> Identity {
    Identity::from_seed(Seed::new([byte; 32]))
}
...
fn signed_dao(unsigned: &[u8], identity: &Identity, sequence: u64) -> Vec<u8> {
    let digest = dao_origin_digest(origin(identity), dodag_id(), sequence, unsigned);
...
fn apply(
    root: &mut DaoManager,
    state: &mut lichen_rpl::routing::DaoRxState,
    storage: &mut MemStorage,
    sender: &mut DaoManager,
    parent: [u8; 16],
    identity: &Identity,
    sequence: u64,
) {
    let wire = signed_dao(&sender.build_dao(parent), identity, sequence);
...
fn downward_routes_assembled_from_daos() {
    ...
    apply(&mut root, &mut state, &mut storage, &mut mgr3, n2, &id3, 1);
    assert_eq!(
    ...
    apply(&mut root, &mut state, &mut storage, &mut mgr4, n2, &id4, 1);
    assert_eq!(
...
fn route_updates_when_node_reparents() {
    ...
    apply(&mut root, &mut state, &mut storage, &mut mgr3, n2, &id3, 1);
    apply(&mut root, &mut state, &mut storage, &mut mgr4, n3, &id4, 1);
    ...
    apply(&mut root, &mut state, &mut storage, &mut mgr4, n2, &id4, 2);

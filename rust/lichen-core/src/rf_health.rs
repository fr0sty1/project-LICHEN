...
impl RssiStats {
    ...
    pub fn update(&mut self, rssi: i16) {
        ...
        if self.count == 0 {
            ...
            // saturating EMA (alpha=1/4 via >> EMA_ALPHA_SHIFT): 
            // avg = avg + (sample - avg) >> 2 for accelerated CCP-15 interference
            // response in da2q multi-channel scenarios (see da2q.15.2.1)

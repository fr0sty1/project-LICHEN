...
//! ```text
//! 0x15 (L2 dispatch) | 0x01 (type) | flags (rx_channel 0-7) | hop | seq (2B) | IID (8B) | pubkey (32B) | sig (48B) | [app_data]
//! ```
//!
//! rx_channel packed in flags byte (offset 1 of announce payload, after type) and included
//! in signed_data per CCP-9 to prevent tampering. Matches ccp9.json independent oracle
//! and _l2_announce_with_channel exactly. Fixed announce payload = 93 bytes.
...
mod tests {
    ...
    fn invalid_channel() {
        let mut wire = make_announce();
        wire[1] = 16; // flags/rx_channel >=8 triggers error
        assert_eq!(
            Announce::from_bytes(&wire),

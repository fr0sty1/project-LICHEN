...
**ASSIGNED_SF and RF Metrics Option (CCP-16):** The DAG Metric Container in DIOs MUST include current SF per-neighbor EMA-derived recommendation per CCP-16 (see spec/02a-coordinated-capacity.md §4.2). Nodes MUST compute TX_SF via pseudocode in physical-link:3.4 and 02a:2a.3. Includes per-neighbor EMA, density, utilization. Test vectors in ccp16.json MUST validate interop. DIOs on control channel provide the announcement. RX scanning on CH0 is REQUIRED.

...
**CCP-16 SF/RF Metrics Option:** The DAG Metric Container in DIO MUST include current SF per-neighbor EMA-derived recommendation per CCP-16 (spec/02a-coordinated-capacity.md §4.2). Nodes MUST compute TX_SF via pseudocode from physical-link:3.4. Includes per-neighbor EMA, density, utilization. DIOs on control channel announce it; RX scanning on CH0 is REQUIRED. Test vectors in ccp16.json MUST validate interop.

...
Advertises node congestion for routing decisions (0-3 scale). Used by objective functions such as MRHOF for route selection.

**Capability and Adaptive SF Option (CCP-16):** The DAG Metric Container and DIO options MUST carry current SF recommendation (or ASSIGNED_SF), per-neighbor EMA-derived metrics (SNR/loss), density, utilization, and load_factor per the normative pseudocode and algorithms in spec/02a-coordinated-capacity.md (adaptive_sf_select, ema_update, select_channel, physical-link:3.4). DIOs MUST be sent on CH0 control channel; nodes MUST RX on all SFs and scan control channel. Nodes MUST compute TX_SF using the spelled-out logic there. Thresholds, EMA parameters (alpha 0.1-0.25), and integration with load_factor are normative. Test vectors in `test/vectors/ccp16*.json`, `ccp_load_balancing.json` and `ccp16-desync.json` are the independent oracles; all implementations MUST match exactly for interoperability. See spec/02a-coordinated-capacity.md §4.2 for full details.

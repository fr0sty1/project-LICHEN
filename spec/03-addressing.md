...
It serves as the primary identifier in UIs, voice communication ("node kilo charlie..."), LCI/CoAP resources (`/keys`, `/status`), RPL/Announce messages (using derived IID or short address). Compatible with OSCORE and link signatures.
...
Cross-references: 04-network.md, 06-security.md, 08-nodes.md, spec/README.md and related I-Ds. Align with `human_address_from_pubkey` (Rust), `lichen_pubkey_to_human_address` (C), and `lichen_pubkey_to_iid()`.

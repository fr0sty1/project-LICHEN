...
This address is short enough to speak, type, and remember. It has acceptable collision probability up to 5B nodes (~0.5 expected collisions at that scale), is cryptographically bound to the Ed25519 pubkey (used for signatures, OSCORE, and IPv6 IID), and compatible with IPv6 addressing. The same IID is used for IPv6 link-local (`fe80::/10`), ULA, and GUA addresses (see 04-network.md). 
...
- Primary identifier in UIs, voice communication ("kilo charlie victor november"), LCI/CoAP resources (/keys, /status)
...
See `test/vectors/node_address.json` and `test/vectors/node-addresses.json`. All implementations (Rust, C, Python, Zephyr) MUST produce identical outputs for given pubkey/seed inputs. Functions like `lichen_pubkey_to_iid` and human address derivation must be consistent across all implementations. Vectors serve as independent oracles; round-trip validation required. Cross-references updated in 04-network.md, 06-security.md, 08-nodes.md, draft-lichen-rpl-lora-00.md, spec/README.md, and related drafts per multi-worker merge.

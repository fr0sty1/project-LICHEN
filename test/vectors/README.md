...
| `rpl_route_state.json` | Canonical grouped DAO route-state transitions and atomic failure snapshots |
| `generate_rpl_route_state.py` | Deterministic fixed-byte route-state vector generator; imports no implementation |
| `validate_rpl_route_state.py` | Independent DAO-byte parser and route-state conformance harness |
...
- Canonical Transit Information uses Path Control `0x80`: with profile PCS=7 this is a nonzero active PC1 preference bit. Mutation vectors override only the field under test.
...
**RPL route state** (`rpl_route_state.json`):
- `vector_type=rpl_route_state` selects a closed dedicated schema. Processing starts after provenance, authorization, and origin replay acceptance so route semantics remain separate from authentication/link behavior.
- Every DAO event carries a complete literal DAO body (excluding the ICMPv6 header and DAO Origin Signature Option), its exact Target/Transit option records, monotonic time, and the complete state before and after the event.
- Initial setup events install parent 2 and parent 3 directly through the root and parent 4 through parent 2. Their infinite lifetimes keep parent reachability independent of later target mutation and expiry tests.
- The child-before-parent sequence first retains child 6's authenticated candidate and freshness state with no selected route. A later literal DAO installs parent 5 through the root, which activates `[parent5, child6]` without a child resend. Incomplete acyclic candidate state is valid; cycles still reject atomically.
- Candidate state records parent, Path Control, Path Sequence, Path Lifetime, installation time, and absolute expiry. The current /128 profile rejects the Transit E flag; equal semantic input MUST NOT refresh either timestamp.
- `sequence_relations` is a fixed 8-bit lollipop truth table derived from RFC 6550 Sections 7.1-7.2. It covers equality, stale and incomparable values, both exact 16/17-step boundaries, cross-region ordering, and the direct `127 -> 0` and `255 -> 0` increments required by the RFC wrap rules.
- `tx_sequence_transitions` fixes cross-implementation DAO generation: counters start at 240, every generated DAO advances DAOSequence, new logical updates advance Path Sequence, and explicit logical copies retain the exact parent, lifetime, and Path Sequence of the preceding successful update.
- `route_hop_boundaries` fixes the complete-route profile limit at eight hops and requires atomic rejection of nine-hop paths. The schema also caps every selected and installed route path at eight entries.
- Target Descriptor is the RFC 6550 Section 6.7.11 `type=0x09`, `length=4`, opaque 32-bit value. It qualifies only the immediately preceding Target; orphan, duplicate, post-Transit, short, or long descriptors reject the DAO atomically. Descriptors are retained in target snapshots but do not alter candidate preference.
- `selected_candidate` gives the selected parent, its PC1-PC4 preference subfield, and complete root-to-egress address sequence, or `null` while no complete acyclic parent chain exists. With profile PCS=7, all eight bits are active; selection uses the first non-empty subfield in PC1, PC2, PC3, PC4 order and then the lexicographically smallest complete path, as required by RFC 6550 Section 9.9 and LICHEN Section 8.8.
- Every snapshot includes `routing_table.routes` with the actual selected complete path, Path Lifetime, installation time, and absolute expiry. Routes are derived only from preceding literal DAO state; no parent path is synthesized by the oracle.
- `disposition` distinguishes active, withdrawn, and expired route records while retaining candidate metadata needed for equal-sequence comparison. The oracle fixes freshness retention at `freshness_retention_seconds`; withdrawal retention starts at receipt, finite-lifetime retention starts at the scheduled expiry deadline, and a tombstone becomes reclaimable exactly at its retention deadline. Reclamation MUST protect every target named by the same atomic DAO, including equal-sequence targets, and may evict only a retained target outside that group.
- Rejected vectors repeat the complete `before` snapshot as `expected.state`, making atomic no-mutation behavior explicit rather than implied by a reason code.
- `expected.reason` is an oracle diagnostic label, not a required public implementation error API. Production harnesses MUST compare acceptance, mutation, and complete state; implementations without detailed rejection enums validate that the canonical diagnostic is present and do not fabricate a reason.
- The scenario covers child-before-parent retention and later activation, cycle rejection, grouped multi-target/multi-parent installation, order-independent equality, every forbidden equal mutation, stale/incomparable sequences, replacement, withdrawal, delayed expiry, exact tombstone reclamation boundaries, malformed groups and descriptors, unsupported external routes, duplicate Targets, and independent target, per-target candidate, and aggregate candidate capacity failures.

The standalone harness parses and applies the literal bytes without importing Rust, Python, or C protocol implementations:

```sh
python3 test/vectors/validate_rpl_route_state.py
```

Production harnesses MUST consume `rpl_route_state.json` directly and compare their resulting route snapshots and selected paths to `expected`; they MUST NOT import the generator or standalone validator. A C harness may generate a native fixture from this JSON, but the JSON remains canonical and generated C output must be reproducible byte-for-byte without editing RPL core files.

```sh
python3 lichen/tests/rpl_routing/generate_rpl_route_state_fixture.py
python3 lichen/tests/rpl_routing/generate_rpl_route_state_fixture.py --check
```

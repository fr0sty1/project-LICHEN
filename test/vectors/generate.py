...
The JSON files written here are language-neutral fixtures consumed by Rust, C,
and Python implementations (see README.md). Independent-oracle files support
conformance claims. Files explicitly classified as implementation-generated
regression baselines detect drift only and do not prove protocol correctness.
...
EDHOC_REGRESSION_DESCRIPTION = (
    "EDHOC deterministic regression fixtures generated by the Python "
    "EdhocInitiator/Responder with fixed seeds. NOT an independent conformance "
    "or interoperability oracle. RFC 9528/9529 correctness is checked against "
    "fixed official literals in python/tests/crypto/test_edhoc.py; use these "
    "vectors only for implementation drift and rejection regressions."
)
...
def l2_payload_vectors() -> list[dict]:
    ...
    return [
        ...
        {
            "name": "malformed_schc_dispatch_only",
            "description": (
                "A defined SCHC dispatch without its required rule ID body byte "
                "is malformed and must fail closed."
            ),
            "dispatch": L2_DISPATCH_SCHC,
            "kind": "unknown",
            "body": "",
            "wrapped": bytes([L2_DISPATCH_SCHC]).hex(),
        },
        {
            "name": "malformed_routing_dispatch_only",
            "description": (
                "A defined routing dispatch without its required message type body "
                "byte is malformed and must fail closed."
            ),
            "dispatch": L2_DISPATCH_ROUTING,
            "kind": "unknown",
            "body": "",
...
def ccp12_synchronized_hop_vectors() -> list[dict]:
    return [
        ...
        },
        {
            ...
            "num_channels": 0,
            "expected_channel": 0,
            "hash_32": _sync_hop_hash(0, 0, 0),
            "description": "An empty channel plan fails closed to CH0 without modulo by zero.",
        },
        {
            "name": "control_only",
            "sfn": 0,
            ...
            "num_channels": 1,
            "expected_channel": 0,
            "hash_32": _sync_hop_hash(0, 0, 1),
            "description": "A CH0-only plan has no data channel and fails closed to CH0.",
        },
        {
            "name": "single_data_channel",
            "sfn": 0,
            "seed": 0,
            "num_channels": 2,
            "expected_channel": 1,
            "hash_32": _sync_hop_hash(0, 0, 2),
            "description": "A two-entry plan always selects its sole data channel CH1.",
        },
        {
            "name": "u8_max_channel_count",
            "sfn": 0,
            "seed": 0,
            "num_channels": 255,
            "expected_channel": _sync_hop_channel(0, 0, 255),
            "hash_32": _sync_hop_hash(0, 0, 255),
            "description": "The maximum cross-language u8 plan count stays within CH0..CH254.",
        },
        {
            "name": "old_modulus_upper_boundary",
            "sfn": 26,
            "seed": 0,
            "num_channels": 64,
            "expected_channel": _sync_hop_channel(26, 0, 64),
            "hash_32": _sync_hop_hash(26, 0, 64),
            "description": "Hash mod 64 equals 63: the obsolete modulus returned out-of-plan CH64; the corrected modulus returns an in-plan data channel.",
        },
        {
...
# Alias for test_vectors.py import pattern consistency with ccp9/13/15/16_vectors
ccp_hop_vectors = ccp12_synchronized_hop_vectors
...
def ccp9_vectors() -> list[dict]:
    ...
    return [
        ...
        },
        {
            ...
            "description": "For known peers, synchronized_hop_channel(seed=epoch, sfn=t) =1 overrides pure announce rendezvous (CCP-12 normative over CCP-9).",
            "eui64_hex": "0011223344556677",
            "t": 1000,
            "epoch": 0,
            "expected_channel": _sync_hop_channel(1000, 0, 8),
            "n_channels": 8,
            "uses_sync_hop": True,
        },
        {
...
def ccp13_vectors() -> list[dict]:
    ...
    return [
        ...
        # AdaptiveDutyPermille vectors per spec 02a.9
        {
            "name": "sparse_region0",
            "description": "AdaptiveDutyPermille(density=0, region=0) per spec 02a.9; crc32 oracle d7292ef6.",
            "density": 0,
            "region": 0,
            "expected_duty_permille": 20,
            "input_crc32": "d7292ef6",
            "notes": "Independent oracle: threshold table arithmetic only, no code-under-test dependency.",
        },
        {
            "name": "sparse_boundary_region0",
            "description": "AdaptiveDutyPermille(density=2, region=0) per spec 02a.9; crc32 oracle d3dcfecb.",
            "density": 2,
            "region": 0,
            "expected_duty_permille": 20,
            "input_crc32": "d3dcfecb",
            "notes": "Independent oracle: threshold table arithmetic only, no code-under-test dependency.",
        },
        {
            "name": "moderate_start_region0",
            "description": "AdaptiveDutyPermille(density=3, region=0) per spec 02a.9; crc32 oracle 3c1e95f5.",
            "density": 3,
            "region": 0,
            "expected_duty_permille": 10,
            "input_crc32": "3c1e95f5",
            "notes": "Independent oracle: threshold table arithmetic only, no code-under-test dependency.",
        },
        {
            "name": "moderate_end_region0",
            "description": "AdaptiveDutyPermille(density=8, region=0) per spec 02a.9; crc32 oracle c4fe6e02.",
            "density": 8,
            "region": 0,
            "expected_duty_permille": 10,
            "input_crc32": "c4fe6e02",
            "notes": "Independent oracle: threshold table arithmetic only, no code-under-test dependency.",
        },
        {
        ...
        {
            "name": "dense_extreme_region0",
            "description": "AdaptiveDutyPermille(density=255, region=0) per spec 02a.9; crc32 oracle 0ea1f4b1.",
            "density": 255,
            "region": 0,
            "expected_duty_permille": 5,
            "input_crc32": "0ea1f4b1",
            "notes": "Independent oracle: threshold table arithmetic only, no code-under-test dependency.",
        },
        {
            "name": "sparse_region1",
            "description": "AdaptiveDutyPermille(density=0, region=1) per spec 02a.9; crc32 oracle a02e1e60.",
            "density": 0,
            "region": 1,
            "expected_duty_permille": 50,
            "input_crc32": "a02e1e60",
            "notes": "Independent oracle: threshold table arithmetic only, no code-under-test dependency.",
        },
        {
            "name": "moderate_region1",
            "description": "AdaptiveDutyPermille(density=5, region=1) per spec 02a.9; crc32 oracle 4607d524.",
            "density": 5,
            "region": 1,
            "expected_duty_permille": 20,
            "input_crc32": "4607d524",
            "notes": "Independent oracle: threshold table arithmetic only, no code-under-test dependency.",
        },
        {
        ...
        {
            "name": "dense_extreme_region1",
            "description": "AdaptiveDutyPermille(density=200, region=1) per spec 02a.9; crc32 oracle eed8a321.",
            "density": 200,
            "region": 1,
            "expected_duty_permille": 10,
            "input_crc32": "eed8a321",
            "notes": "Independent oracle: threshold table arithmetic only, no code-under-test dependency.",
        },
    ]
    ...
    return [
        ...
        },
        {
            ...
            "description": (
                ...
                "(RFC 6550 6.7.9): type 7 length 19, RPLInstanceID 0, VID flags "
        ...
        {
            "name": "dao_ack_status_alternate_parent_boundary",
            "type": "dao_ack",
            "description": "Status 127 is preserved as the upper alternate-parent recommendation boundary.",
            "encoded": "0000077f",
            "fields": {"rpl_instance_id": 0, "flags": 0, "dao_sequence": 7, "status": 127, "dodag_id": None},
        },
        {
            "name": "dao_ack_status_rejection_boundary",
            "type": "dao_ack",
            "description": "Status 128 is preserved as the lower outright-rejection boundary.",
            "encoded": "00000880",
            "fields": {"rpl_instance_id": 0, "flags": 0, "dao_sequence": 8, "status": 128, "dodag_id": None},
        },
        {
            "name": "dao_ack_status_maximum",
            "type": "dao_ack",
            "description": "The complete Status octet is significant, including rejection code 255.",
            "encoded": "000009ff",
            "fields": {"rpl_instance_id": 0, "flags": 0, "dao_sequence": 9, "status": 255, "dodag_id": None},
        },
        {
            "name": "dao_ack_nonzero_reserved_flags",
            "type": "dao_ack",
            "description": "Reserved DAO-ACK flag bits are rejected by the strict LICHEN wire profile.",
            "encoded": "00010500",
            "expect_error": "nonzero_reserved_flags",
        },
        {
            "name": "dao_ack_malformed_options_d0",
            "type": "dao_ack",
            "description": "D=0 followed by a truncated type-5 option is rejected.",
            "encoded": "00000500050401",
        ...
        {
            "name": "dodag_config_type4_default",
            "type": "option",
            "option_type": 4,
            "description": "Canonical DODAG Configuration with the LICHEN defaults and all flags clear.",
            "encoded": "040e00080c0a08000100000100ff003c",
            "fields": {
                "pcs": 0,
        ...
        {
            "name": "dodag_config_type4_all_defined_flags",
            "type": "option",
            "option_type": 4,
            "description": "Gateway-centric, RFC A, and PCS fields occupy bits 7, 3, and 2..0 respectively and every numeric field round-trips.",
            "encoded": "040e8d070b03123402000002001e0e10",
            "fields": {
                "pcs": 5,
        ...
        {
            "name": "dodag_config_type4_short_data",
            "type": "option",
            "option_type": 4,
            "description": "Data Length 13 truncates Lifetime Unit and is rejected.",
            "encoded": "040d00080c0a08000100000100ff00",
            "expect_error": "invalid_length",
        },
        {
            "name": "dodag_config_type4_trailing_data",
            "type": "option",
            "option_type": 4,
            "description": "Data Length 15 is rejected rather than partially parsing a canonical option plus trailing data.",
            "encoded": "040f00080c0a08000100000100ff003c00",
            "expect_error": "invalid_length",
        },
        {
            "name": "dodag_config_type4_reserved_flag",
            "type": "option",
            "option_type": 4,
            "description": "LICHEN reserves flag bits 6..4 and rejects them before state mutation.",
            "encoded": "040e40080c0a08000100000100ff003c",
            "expect_error": "nonzero_reserved_flags",
        },
        {
            "name": "dodag_config_type4_reserved_octet",
            "type": "option",
            "option_type": 4,
            "description": "The Reserved octet after OCP must be zero.",
            "encoded": "040e00080c0a08000100000101ff003c",
            "expect_error": "nonzero_reserved_octet",
        },
        {
            "name": "rpl_target_type5_host",
            "type": "option",
            "option_type": 5,
            "description": "Canonical LICHEN RPL Target: Data Length 18, zero flags, Prefix Length 128, and a complete IPv6 address.",
            "encoded": "0512008002000000000000000000000000000001",
            "fields": {"prefix_length": 128, "prefix": "200::1"},
        },
        {
            "name": "rpl_target_type5_nonzero_flags",
            "type": "option",
            "option_type": 5,
            "description": "LICHEN reserves all RPL Target flags and rejects any nonzero value.",
            "encoded": "0512018002000000000000000000000000000001",
            "expect_error": "nonzero_flags",
        },
        {
            "name": "rpl_target_type5_short_data",
            "type": "option",
            "option_type": 5,
            "description": "A type-5 option with Data Length 17 is not the canonical /128 LICHEN profile.",
            "encoded": "05110080020000000000000000000000000000",
            "expect_error": "invalid_length",
        },
        {
            "name": "rpl_target_type5_trailing_data",
            "type": "option",
            "option_type": 5,
            "description": "A type-5 option with trailing data and Data Length 19 is rejected rather than partially parsed.",
            "encoded": "051300800200000000000000000000000000000100",
            "expect_error": "invalid_length",
        },
        {
            "name": "rpl_target_type5_non_host_prefix",
            "type": "option",
            "option_type": 5,
            "description": "The LICHEN DAO profile permits only host routes, so Prefix Length 127 is rejected even with Data Length 18.",
            "encoded": "0512007f02000000000000000000000000000001",
            "expect_error": "invalid_prefix_length",
        },
        {
            "name": "transit_type6_node_owned",
            "type": "option",
            "option_type": 6,
            "description": "Canonical node-owned Transit Information: Data Length 20, E clear, and the mandatory Parent Address.",
            "encoded": "0614008007fffe800000000000000000000000000001",
            "fields": {
                "external": False,
                "path_control": 128,
                "path_sequence": 7,
                "path_lifetime": 255,
                "parent_address": "fe80::1",
            },
        },
        {
            "name": "transit_type6_external",
            "type": "option",
            "option_type": 6,
            "description": "The E bit represents external reachability while the Parent Address remains mandatory.",
            "encoded": "06148040091efe800000000000000000000000000002",
            "fields": {
                "external": True,
                "path_control": 64,
                "path_sequence": 9,
                "path_lifetime": 30,
                "parent_address": "fe80::2",
            },
        },
        {
            "name": "transit_type6_short_data",
            "type": "option",
            "option_type": 6,
            "description": "Data Length 19 truncates the mandatory Parent Address and is rejected.",
            "encoded": "0613008007fffe8000000000000000000000000000",
            "expect_error": "invalid_length",
        },
        {
            "name": "transit_type6_trailing_data",
            "type": "option",
            "option_type": 6,
            "description": "Data Length 21 is rejected rather than partially parsing a canonical option plus trailing data.",
            "encoded": "0615008007fffe80000000000000000000000000000100",
            "expect_error": "invalid_length",
        },
        {
            "name": "transit_type6_missing_parent",
            "type": "option",
            "option_type": 6,
            "description": "The RFC four-byte form is outside the LICHEN non-storing profile because it omits the Parent Address.",
            "encoded": "0604008007ff",
            "expect_error": "invalid_length",
        },
        {
            "name": "transit_type6_reserved_flag",
            "type": "option",
            "option_type": 6,
            "description": "Reserved Transit flag bits must be zero; E is the only defined flag.",
            "encoded": "0614018007fffe800000000000000000000000000001",
            "expect_error": "nonzero_reserved_flags",
        },
    ]
...
def tofu_edge_cases_vectors() -> list[dict]:
    """Canonical TOFU state-machine edge cases shared by all runtimes.

    The public keys and IIDs are fixed independent literals from the GCP-3
    corpus.  Results intentionally describe security and state invariants,
    not language-specific exception or errno names.
    """
    alice = "4cb5abf6ad79fbf5abbccafcc269d85cd2651ed4b885b5869f241aedf0a5ba29"
    alice_iid = "fd6b265c8585369b"
    bob = "7422b9887598068e32c4448a949adb290d0f4e35b9e01b0ee5f1a1e600fe2674"
    bob_iid = "888bcf64cfefa304"
    ...
    def case(
        ...
        *,
        ...
        vector = {
            ...
            "concurrent_observers": concurrent,
        ...
        if snapshot_generation is not None:
            vector["snapshot_generation"] = snapshot_generation
        if minimum_generation is not None:
            vector["minimum_generation"] = minimum_generation
        if durable_commits is not None:
            vector["expected"]["durable_commits"] = durable_commits
        return vector

    return [
        case("first_contact", "contact", "A valid first observation is pinned before it is accepted.", initial="empty", operation="observe", result="pin_and_accept", entry_count=1, state_unchanged=False, durable_commits=1),
        case("idempotent_repeat", "contact", "Repeating the pinned identity accepts it without adding a second entry.", initial="alice_pinned", operation="observe", result="accept_known", entry_count=1, state_unchanged=True, repetitions=2),
        case("derivation_mismatch", "identity_binding", "A valid key claiming another key's IID is rejected before pinning.", initial="empty", operation="observe", pubkey=bob, iid=alice_iid, result="reject_identity_conflict", entry_count=0, state_unchanged=True),
        case("pinned_key_collision", "identity_binding", "A different key cannot replace an existing IID pin.", initial="alice_pinned", operation="observe", pubkey=bob, iid=alice_iid, result="reject_identity_conflict", entry_count=1, state_unchanged=True),
        case("malformed_pubkey_short", "malformed", "A 31-byte public key is rejected without mutation.", initial="empty", operation="observe", pubkey=alice[:-2], result="reject_malformed", entry_count=0, state_unchanged=True),
        case("malformed_pubkey_long", "malformed", "A 33-byte public key is rejected without mutation.", initial="empty", operation="observe", pubkey=alice + "00", result="reject_malformed", entry_count=0, state_unchanged=True),
        case("malformed_iid_short", "malformed", "A 7-byte IID is rejected without mutation.", initial="empty", operation="observe", iid=alice_iid[:-2], result="reject_malformed", entry_count=0, state_unchanged=True),
        case("malformed_iid_long", "malformed", "A 9-byte IID is rejected without mutation.", initial="empty", operation="observe", iid=alice_iid + "00", result="reject_malformed", entry_count=0, state_unchanged=True),
        case("persistence_failure_rollback", "persistence", "A failed durable commit rolls the candidate pin back before returning.", initial="empty", operation="observe", persistence="fail", result="reject_persistence", entry_count=0, state_unchanged=True, durable_commits=0),
        case("reboot_restore", "persistence", "A committed pin is authenticated and restored after reboot.", initial="alice_pinned", operation="reboot", result="restore", entry_count=1, state_unchanged=True, snapshot_generation=2, minimum_generation=2),
        case("rollback_snapshot_rejected", "persistence", "A snapshot below the independently stored generation floor is rejected.", initial="alice_pinned", operation="reboot", result="reject_rollback", entry_count=0, state_unchanged=True, snapshot_generation=1, minimum_generation=2),
        case("concurrent_first_contact", "concurrency", "Concurrent identical first observations converge to one pin and one durable commit.", initial="empty", operation="observe", result="single_pin", entry_count=1, state_unchanged=False, concurrent=2, durable_commits=1),
        case("replayed_collision", "replay", "Repeated conflicting observations remain rejected and never overwrite the pin.", initial="alice_pinned", operation="observe", pubkey=bob, iid=alice_iid, result="reject_identity_conflict", entry_count=1, state_unchanged=True, repetitions=3),
        case("independent_peer", "contact", "A second correctly bound identity is an independent pin, not a collision.", initial="alice_pinned", operation="observe", pubkey=bob, iid=bob_iid, result="pin_and_accept", entry_count=2, state_unchanged=False, durable_commits=1),
    ]
...
VECTOR_FILES: tuple[_VectorFile, ...] = (
    ...
    _VectorFile(
        ...
        "Cross-language TOFU state-machine edge cases covering contact, binding, malformed input, durable rollback, reboot, concurrency, and replay.",
        builder="tofu_edge_cases_vectors",
        name="tofu_edge_cases",
        spec="spec/06-security.md#trust-on-first-use-tofu",
    ),
    _VectorFile(
...
def edhoc_vectors() -> list[dict]:
    """Build Python-generated EDHOC drift and rejection regression fixtures.

    These fixtures use the production Python implementation itself and are not
    an independent correctness or interoperability oracle. Official RFC
    9528/9529 fixed literals are tested separately in test_edhoc.py.
    """
    import os

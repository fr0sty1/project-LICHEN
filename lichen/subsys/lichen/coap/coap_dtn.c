			 ...
			 struct sockaddr *addr, socklen_t addr_len)
{
	...
	uint8_t iid_idx = hash_from_iid(peer_eui64);
	k_mutex_lock(&s_rate_mutex, K_FOREVER);
	if (s_last_deaddrop[iid_idx] &&
	    (now_ms - s_last_deaddrop[iid_idx] <
	...
	s_last_deaddrop[iid_idx] = now_ms;
	k_mutex_unlock(&s_rate_mutex);
	k_mutex_lock(&s_dtn_buf_mutex, K_FOREVER);
			    ...
			    struct sockaddr *addr, socklen_t addr_len)
{
	...
	uint8_t iid_idx = hash_from_iid(peer_eui64);
	k_mutex_lock(&s_rate_mutex, K_FOREVER);
	if (s_last_confession[iid_idx] &&
	    (now_ms - s_last_confession[iid_idx] <
	     CONFIG_LICHEN_COAP_DEADDROP_RATE_LIMIT_MS)) {
		k_mutex_unlock(&s_rate_mutex);
#ifdef CONFIG_LICHEN_COAP_SERVER_OSCORE
		if (is_protected && ctx != NULL) {
	...
	s_last_confession[iid_idx] = now_ms;
	k_mutex_unlock(&s_rate_mutex);

			 ...
			 struct sockaddr *addr, socklen_t addr_len)
{
	...
	if (s_last_deaddrop[iid7] &&
		...
		return lichen_coap_oscore_respond(resource, request, addr, addr_len,
						  &auth, COAP_RESPONSE_CODE_TOO_MANY_REQUESTS);
	}
	s_last_deaddrop[iid7] = now_ms;
	k_mutex_unlock(&s_rate_mutex);
	k_mutex_lock(&s_dtn_buf_mutex, K_FOREVER);
	if (s_provider && s_provider->store) {
		int r = s_provider->store(auth.payload, auth.payload_len);
		if (r < 0) {
			k_mutex_unlock(&s_dtn_buf_mutex);
			return lichen_coap_oscore_respond(resource, request, addr, addr_len,
	...
	bool ok = lichen_dtn_buffer_message(&s_dtn_buf, auth.payload, auth.payload_len,
			    ...
			    struct sockaddr *addr, socklen_t addr_len)
{
	...
	/* Authorize: OSCORE ctx lookup + unprotect, or fall through to raw payload.
	 * confessions accepts both OSCORE-protected and unprotected mesh-originated
	 * requests (no is_local_admin check needed). */
	ret = lichen_coap_oscore_authorize_mutating(resource, request, addr, addr_len,
						    COAP_METHOD_POST, &auth,
						    plain, sizeof(plain));
	if (ret < 0) {

...
static int deaddrop_post(struct coap_resource *resource, struct coap_packet *request, struct sockaddr *addr, socklen_t addr_len) {
	...
	if (!ok) {
		if (is_protected && ctx != NULL && piv_len > 0) {
			return deaddrop_oscore_respond(resource, request, addr, addr_len, ctx, piv, piv_len, COAP_RESPONSE_CODE_BAD_REQUEST);
		}
		return COAP_RESPONSE_CODE_BAD_REQUEST;
	}
	if (is_protected && ctx != NULL && piv_len > 0) {
		return deaddrop_oscore_respond(resource, request, addr, addr_len, ctx, piv, piv_len, COAP_RESPONSE_CODE_CHANGED);
	}

			   ...
			   struct sockaddr *addr, socklen_t addr_len)
{
	...
	enum lichen_key_trust trust;
	struct lichen_coap_oscore_auth_result auth;
	uint8_t plain[LICHEN_COAP_SERVER_MAX_PAYLOAD];
	int ret;

	/* Authorize: OSCORE ctx lookup + unprotect, or fall through to local admin */
	ret = lichen_coap_oscore_authorize_mutating(resource, request, addr, addr_len,
						    COAP_METHOD_PUT, &auth,
						    plain, sizeof(plain));
	if (ret < 0) {
		return ret;
	}
	 ...
	 * OSCORE-protected requests skip this check (already authenticated via ctx).
	 */
	if (!auth.is_protected && !lichen_coap_is_local_admin(addr, addr_len)) {
		LOG_WRN("PUT /keys rejected: not local admin");
		return lichen_coap_respond(resource, request, addr, addr_len,
				    COAP_RESPONSE_CODE_UNAUTHORIZED, 0, NULL, 0);
	}

	int opt_count = coap_find_options(request, COAP_OPTION_URI_PATH, options, ARRAY_SIZE(options));
	if (opt_count < 2) {
		return lichen_coap_oscore_respond(resource, request, addr, addr_len,
						  &auth, COAP_RESPONSE_CODE_BAD_REQUEST);
	}
	...
	ret = decode_key_put_cbor(auth.payload, auth.payload_len, pubkey, &trust);
	if (ret < 0) {
		return lichen_coap_oscore_respond(resource, request, addr, addr_len,
						  &auth, COAP_RESPONSE_CODE_BAD_REQUEST);
	}
	...
	if (ret == -EEXIST) {
		return lichen_coap_oscore_respond(resource, request, addr, addr_len,
						  &auth, COAP_RESPONSE_CODE_CONFLICT);
	}
	if (ret == -ENOSPC) {
		return lichen_coap_oscore_respond(resource, request, addr, addr_len,
						  &auth, COAP_RESPONSE_CODE_SERVICE_UNAVAILABLE);
	}
	if (ret < 0) {
		return lichen_coap_oscore_respond(resource, request, addr, addr_len,
						  &auth, COAP_RESPONSE_CODE_INTERNAL_ERROR);
	}

	LOG_INF("Key added/updated for IID %s", iid_str);
	return lichen_coap_oscore_respond(resource, request, addr, addr_len,
					  &auth, COAP_RESPONSE_CODE_CHANGED);
}
			      ...
			      struct sockaddr *addr, socklen_t addr_len)
{
	...
	/* Authorize: OSCORE ctx lookup + unprotect, or fall through to local admin */
	ret = lichen_coap_oscore_authorize_mutating(resource, request, addr, addr_len,
						    COAP_METHOD_DELETE, &auth,
						    plain, sizeof(plain));
	if (ret < 0) {
		return ret;
	}

	/* SECURITY: Require local admin access for delete operations.
	 * OSCORE-protected requests skip this check (already authenticated via ctx).
	 */
	if (!auth.is_protected && !lichen_coap_is_local_admin(addr, addr_len)) {
		LOG_WRN("DELETE /keys rejected: not local admin");
		return lichen_coap_oscore_respond(resource, request, addr, addr_len,
						  &auth, COAP_RESPONSE_CODE_UNAUTHORIZED);
	}
	...
	if (ret == -ENOENT) {
		return lichen_coap_oscore_respond(resource, request, addr, addr_len,
						  &auth, COAP_RESPONSE_CODE_NOT_FOUND);
	}
	if (ret < 0) {
		return lichen_coap_oscore_respond(resource, request, addr, addr_len,
						  &auth, COAP_RESPONSE_CODE_INTERNAL_ERROR);
	...
	return lichen_coap_oscore_respond(resource, request, addr, addr_len,
					  &auth, COAP_RESPONSE_CODE_DELETED);
}

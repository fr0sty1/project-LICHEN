...
/** Atomically copy signing material. Caller MUST clear the snapshot after use. */
int lichen_link_snapshot_keypair(
	const struct lichen_link_ctx *_Nonnull ctx,
	struct lichen_link_keypair_snapshot *_Nonnull snapshot);

/** Securely clear a signing identity snapshot. */
void lichen_link_clear_keypair_snapshot(
	struct lichen_link_keypair_snapshot *_Nullable snapshot);

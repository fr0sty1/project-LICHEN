...
int lichen_link_init(struct lichen_link_ctx *ctx, const uint8_t *eui64)
{
	 ...
	 * The epoch drives key rotation only; TDMA slots come from
	 * Slot=(fnv1a32(EUI64)+u32(SFN)) mod n per spec/02a-coordinated-capacity.md
	 * §2a.2 (ccp16.json, ccp_tdma.json), so the epoch MUST NOT enter the hash.
	...
	/* Reset the TX-gate TDMA latch so the first signed frame after init
	 * performs a fresh lichen_tdma_init() for the epoch load_key() will
	 * install. Explicit reset keeps stack-allocated contexts
	 * deterministic even though no blanket memset exists. */
	memset(&ctx->tdma, 0, sizeof(ctx->tdma));
	ctx->tdma_epoch_seen = 0;
...
int lichen_link_next_tx(struct lichen_link_ctx *ctx, uint8_t *epoch, uint16_t *seqnum)
{
	 ...
	 * rotation occurs. Continuing would allow (epoch, seqnum) replay
	 * state to be reused, breaking the anti-replay guarantees peers
	 * rely on for their replay windows.
	 */
 ...
 *
 * For Monocypher builds, uses crypto_wipe(). For POSIX builds, uses
 * explicit_bzero() which is guaranteed not to be optimized away.
 */
static void secure_wipe(void *buf, size_t len)
{
#ifdef CONFIG_LICHEN_CRYPTO_MONOCYPHER
...
#ifdef CONFIG_LICHEN_TDMA
	/* Drop any schedule state latched before cleanup so a later load_key()
	 * cannot collide with tdma_epoch_seen and skip re-initialization. */
	memset(&ctx->tdma, 0, sizeof(ctx->tdma));
	ctx->tdma_epoch_seen = 0;
	ctx->tdma_init_done = false;
#endif

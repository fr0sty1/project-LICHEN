...
int lichen_rpl_dao_manager_build_dao_with_lifetime(
	struct lichen_rpl_dao_manager *dm, const uint8_t *parent_addr,
	uint8_t path_lifetime, uint8_t *buf, size_t len)
{
	if (dm == NULL) {
		return LICHEN_RPL_ERR_INVALID;
	}
	...
	uint8_t path_lifetime, uint8_t *buf, size_t len)
{
	...
	k_mutex_lock(&dm->lock, K_FOREVER);
	if (!dm->has_last_dao_update || parent_addr == NULL ||
...
static int candidate_compare(const struct lichen_rpl_dao_candidate *a,
			     const struct lichen_rpl_dao_candidate *b)
{
	int cmp = memcmp(a->parent, b->parent, 16);

	if (cmp != 0) {
		return cmp;
	}
	if (a->path_control != b->path_control) {
		return a->path_control < b->path_control ? -1 : 1;
	}
	if (a->path_lifetime != b->path_lifetime) {
		return a->path_lifetime < b->path_lifetime ? -1 : 1;
	}
	return (int)a->external - (int)b->external;
}
			   ...
			   const struct lichen_rpl_dao_snapshot *b)
{
	if (a->candidate_count != b->candidate_count ||
	    a->has_descriptor != b->has_descriptor ||
	    (a->has_descriptor && a->descriptor != b->descriptor)) {
		return false;
	}
	...
	uint8_t incoming, uint8_t current)
{
	...
	if (incoming < 128 && current < 128) {
		uint8_t forward = (incoming - current) & 0x7fU;
		uint8_t backward = (current - incoming) & 0x7fU;

		if (forward <= 16) {
			return LICHEN_RPL_SEQUENCE_NEWER;
		}
		return backward <= 16 ? LICHEN_RPL_SEQUENCE_STALE :
			LICHEN_RPL_SEQUENCE_INCOMPARABLE;
	}
	if (incoming >= 128 && current >= 128) {
		int difference = (int)incoming - (int)current;

		if (difference >= 1 && difference <= 16) {
			return LICHEN_RPL_SEQUENCE_NEWER;
		}
		return difference >= -16 && difference <= -1 ?
			LICHEN_RPL_SEQUENCE_STALE : LICHEN_RPL_SEQUENCE_INCOMPARABLE;
	}
	if (incoming < 128) {
		return 256U + incoming - current <= 16U ?
			LICHEN_RPL_SEQUENCE_NEWER : LICHEN_RPL_SEQUENCE_STALE;
	}
	return 256U + current - incoming <= 16U ?
		LICHEN_RPL_SEQUENCE_STALE : LICHEN_RPL_SEQUENCE_NEWER;
}
...
static bool finish_group(struct lichen_rpl_dao_stage *staged, int *staged_count,
			 ...
			 int target_count,
			 struct lichen_rpl_dao_candidate *candidates,
			 int candidate_count, uint8_t path_sequence)
{
	if (target_count == 0 || candidate_count == 0 ||
	    *staged_count + target_count > CONFIG_LICHEN_RPL_MAX_ROUTES) {
		return false;
	}

	for (int i = 1; i < candidate_count; i++) {
		struct lichen_rpl_dao_candidate value = candidates[i];
		int j = i;

		while (j > 0 && candidate_compare(&value, &candidates[j - 1]) < 0) {
	...
	for (int i = 0; i < target_count; i++) {
		for (int j = 0; j < *staged_count; j++) {
			if (rpl_addr_eq(staged[j].snapshot.target, targets[i].target)) {
				return false;
			}
		}
		struct lichen_rpl_dao_snapshot *snapshot =
			&staged[*staged_count].snapshot;

		memset(snapshot, 0, sizeof(*snapshot));
		rpl_addr_copy(snapshot->target, targets[i].target);
		snapshot->has_descriptor = targets[i].has_descriptor;
		snapshot->descriptor = targets[i].descriptor;
		memcpy(snapshot->candidates, candidates,
		       (size_t)candidate_count * sizeof(candidates[0]));
		snapshot->candidate_count = (uint8_t)candidate_count;
		snapshot->path_sequence = path_sequence;
		staged[*staged_count].slot = -1;
		staged[*staged_count].changed = false;
		(*staged_count)++;
	}
	return true;
}
				    ...
				    uint32_t now, uint32_t lifetime_unit)
{
	...
	for (int i = 0; i < CONFIG_LICHEN_RPL_MAX_ROUTES; i++) {
		...
		uint64_t max_age = (uint64_t)r->path_lifetime * lifetime_unit;
		uint32_t deadline = r->last_updated + (uint32_t)max_age;
				  ...
				  uint32_t now, uint32_t lifetime_unit)
{
	...
	for (int i = 0; i < CONFIG_LICHEN_RPL_MAX_ROUTES; i++) {
		...
		uint64_t max_age = (uint64_t)lifetime * lifetime_unit;
		uint32_t deadline = snapshot->last_updated + (uint32_t)max_age;
		if (time_reached(now, deadline)) {
			snapshot->active = false;
			snapshot->disposition = LICHEN_RPL_DAO_EXPIRED;
			snapshot->retain_until = retain_deadline(deadline);
			expired++;
		}
	}
	if (expired > 0) {

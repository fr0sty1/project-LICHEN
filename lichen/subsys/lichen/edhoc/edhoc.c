				 ...
				 size_t *msg3_len)
{
	...
	/* TH_2 = H(CBOR(bstr(G_Y)), CBOR(bstr(H(message_1))), CBOR(bstr(C_R))) per RFC 9528 Section 4.1.2 */
	uint8_t h_msg1[32];
	...
	ret = compute_th(ctx->th_2, ctx->g_y, 32, h_msg1, 32,
			 ctx->c_r, ctx->c_r_len);
	if (ret != 0) {
		goto err_wipe;
	}
				 ...
				 size_t *msg2_len)
{
	...
	ret = compute_th(ctx->th_2, ctx->eph_pk, 32, h_msg1, 32,
			 ctx->c_r, ctx->c_r_len);
	if (ret != 0) {
		goto err_wipe;
	}

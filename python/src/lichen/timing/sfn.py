...
# Guard time and slot duration per §14.8
TDMA_GUARD_MS: int = 50  # §14.8 text
TDMA_GUARD_MS_ALT: int = 100  # Table in §14.8 states 100ms enforced per §2a.2 (both documented)
TDMA_SLOT_MS: int = 250  # airtime + guard (SF10/125kHz ≈250ms)
...
def slot_for(eui64: bytes, sfn: int, num_slots: int) -> int:
    """Return slot index: (hash_32(eui64) + sfn) mod num_slots per spec §14.7."""
    if len(eui64) != 8:
        raise ValueError("eui64 must be 8 bytes")
    if num_slots <= 0:
        raise ValueError("num_slots must be positive")
    return (hash_32(eui64) + (sfn & 0xFFFFFFFF)) % num_slots

...
class ConfigResource(_ReadResource):
    ...
    def __init__(self, node_info: NodeInfo, *, allow_config_write: bool = False) -> None:
        super().__init__(node_info)
        self._allow_config_write = allow_config_write

    async def render_get(self, request: Message) -> Message:
        return _cbor_response(self.node_info.get_config())

    async def render_put(self, request: Message) -> Message:
        if not self._allow_config_write:
    ...
    **GET** returns the inbox (most recent :data:`_MESSAGES_MAX` messages, oldest
    ...
    async def render_post(self, request: Message) -> Message:
        ...
        candidate_inbox = [
            ...
            if not (type(message.get("id")) is int and message["id"] == body["id"])
        ]
        candidate_inbox = [*candidate_inbox, body][-self._max_messages:]
        candidate_sent = dict(self._sent)
        candidate_order = list(self._sent_order)
        if msg_id in candidate_sent:
            candidate_order.remove(msg_id)
        candidate_sent[msg_id] = copy.deepcopy(body)
        candidate_order.append(msg_id)
        while len(candidate_order) > self._max_messages:
            evicted_id = candidate_order.pop(0)
            del candidate_sent[evicted_id]

        self._next_id = candidate_next_id
        self._inbox = candidate_inbox
        self._sent = candidate_sent
        self._sent_order = candidate_order
        self.updated_state()
        for alias in self._legacy_aliases:
            alias.updated_state()
...
class EdhocResource(resource.Resource):
    ...
    async def render_post(self, request: Message) -> Message:
        ...
        try:
            ...
            if payload == session["msg1"]:
                return self._edhoc_response(session["msg2"])
...
def build_site(
    ...
    *,
    ...
    site.add_resource(
        ["config"], ConfigResource(node_info, allow_config_write=allow_config_write)
    )

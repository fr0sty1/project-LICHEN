...
_STRING_FIELDS = frozenset(("bn", "bu", "n", "u", "vs"))
_NUMERIC_FIELDS = frozenset(("bt", "bv", "bs", "v", "s", "t", "ut"))


def _validate_field_type(name: str, value: object) -> None:
    """Validate a decoded field's RFC 8428 value type."""
    if name in _STRING_FIELDS and type(value) is not str:
        expected = "a string"
    elif name in _NUMERIC_FIELDS and type(value) not in (int, float, Decimal):
        expected = "a number"
    elif name == "bver" and type(value) is not int:
        expected = "an integer"
    elif name == "vb" and type(value) is not bool:
        expected = "a boolean"
    elif name == "vd" and type(value) is not bytes:
        expected = "bytes"
    else:
        return
    raise ValueError(f"SenML field '{name}' must be {expected}, got {type(value).__name__}")
...
class SenmlRecord:
    ...
    * ``ut`` — update time (seconds)
    ...
    @classmethod
    def from_cbor_map(cls, m: Mapping[Any, Any]) -> SenmlRecord:

...
"""RPL DAO handling for non-storing mode (RFC 6550, spec section 8.8)."""
...
_MAX_CHAIN = 64
_MAX_ROUTE_HOPS = 8
_TARGET_DESCRIPTOR = 9
_DEFAULT_FRESHNESS_RETENTION_SECONDS = 60 * 60
...
class RplTarget:
    ...
    def to_option(self) -> RplOption:
        if not 0 <= self.prefix_length <= 128:
            raise DaoError(f"RPL Target prefix length out of range: {self.prefix_length}")
    ...
    def from_option(cls, opt: RplOption) -> RplTarget:
        ...
        if prefix_length > 128:
            raise DaoError(f"RPL Target prefix length out of range: {prefix_length}")
...
class TransitInformation:
    """Transit Information option carrying one candidate parent."""
    ...
    def to_option(self) -> RplOption:
        if self.external:
            raise DaoError(
                "external Transit Information is not supported",
    ...
    def from_option(cls, opt: RplOption) -> TransitInformation:
        ...
        if len(opt.data) != 20:
            raise DaoError("Transit Information option must contain a parent address")
        if opt.data[0] != 0:
            raise DaoError(
                "Transit Information flags must be zero",
                reason="unsupported_external",
            )
        return cls(
...
class DaoManager:
    ...
    def process_dao(self, dao: DAO) -> DAOAck | None:
        """Validate and atomically apply a DAO using the configured clock."""
        return self.process_dao_at(dao, self.clock())

...
def test_newer_replaces_complete_snapshot_equal_is_idempotent_and_stale_rejects() -> None:
    root = DaoManager(node_address=ROOT, is_root=True, lifetime_unit_seconds=1)
    root.process_dao_at(make_dao(N1, ROOT), 0)
    root.process_dao_at(make_dao(N2, ROOT), 0)
    first = make_dao(N3, N1, sequence=10, lifetime=10)
    root.process_dao_at(first, 100)
    assert root.routing_table.lookup(N3) == [N1, N3]

    root.process_dao_at(make_dao(N3, N1, sequence=10, lifetime=10), 105)
    assert root._edge_expiry[(N3, N1)] == 110
    with pytest.raises(DaoError, match="equal Path Sequence"):
        root.process_dao_at(make_dao(N3, N2, sequence=10, lifetime=10), 106)
    with pytest.raises(DaoError, match="stale or incomparable"):
        root.process_dao_at(make_dao(N3, N2, sequence=9, lifetime=10), 106)

    root.process_dao_at(make_dao(N3, N2, sequence=11, lifetime=20), 106)
...
def test_expired_tombstone_reclaims_only_after_retention_boundary() -> None:
    ...
    root.process_dao_at(make_dao(N2, ROOT), 112)
    assert root._path_sequences == {N2: 1}
    assert N1 not in root._candidate_map
    assert root.routing_table.lookup(N2) == [N2]


def test_withdrawal_tombstone_reclaims_after_retention_and_failure_rolls_back() -> None:
    root = DaoManager(
        node_address=ROOT,
        is_root=True,
        freshness_retention_seconds=10,
    ...
    root.process_dao_at(make_dao(N1, ROOT, sequence=1, lifetime=0), 20)
    with pytest.raises(DaoError, match="capacity"):
        root.process_dao_at(make_dao(N2, ROOT), 29.999)

    with pytest.raises(DaoError, match="no active Path Control"):
        root.process_dao_at(make_dao(N2, ROOT, control=0), 30)
    assert root._path_sequences == {N1: 1}
    assert N1 in root._candidate_map

    root.process_dao_at(make_dao(N2, ROOT), 30)
    assert root._path_sequences == {N2: 1}


def test_grouped_dao_reclaims_two_oldest_tombstones_atomically() -> None:
    old_targets = [IPv6Address(f"fd00::{index}") for index in range(21, 24)]
    new_targets = [IPv6Address(f"fd00::{index}") for index in range(31, 33)]
    root = DaoManager(
        node_address=ROOT,
        is_root=True,
        freshness_retention_seconds=10,
    ...
    for target, now in zip(old_targets, [10, 11, 12], strict=True):
        root.process_dao_at(make_dao(target, ROOT, lifetime=0), now)
    assert all(record.reclaimable(22) for record in root._freshness.values())

    root.process_dao_at(
        grouped_dao(new_targets, [TransitInformation(ROOT, path_sequence=2)]),
        22,
    )

    assert set(root._path_sequences) == {old_targets[2], *new_targets}
    assert len(root._path_sequences) == root.max_targets
    assert len(root._freshness) == root.max_targets
    assert set(root._candidate_map) == set(root._path_sequences)
    assert root.routing_table.lookup(old_targets[2]) is None
    assert root.routing_table.lookup(new_targets[0]) == [new_targets[0]]
    assert root.routing_table.lookup(new_targets[1]) == [new_targets[1]]


def test_grouped_dao_rejects_when_one_too_few_tombstones_are_reclaimable() -> None:
    old_targets = [IPv6Address(f"fd00::{index}") for index in range(21, 24)]
    new_targets = [IPv6Address(f"fd00::{index}") for index in range(31, 33)]
    root = DaoManager(
        node_address=ROOT,
    ...
    for target, now in zip(old_targets, [10, 11, 12], strict=True):
        root.process_dao_at(make_dao(target, ROOT, lifetime=0), now)
    assert sum(record.reclaimable(20) for record in root._freshness.values()) == 1
    before = (
        dict(root._path_sequences),
        dict(root._candidate_map),
        dict(root._descriptors),
        dict(root._candidate_timing),
        dict(root._freshness),
        dict(root._parent_map),
        dict(root._edge_expiry),
        root.routing_table_snapshot(),
    )
    ...
    assert set(root._path_sequences) == set(old_targets)
    assert all(root.routing_table.lookup(target) is None for target in new_targets)


def test_pcs_masks_low_bits_and_compares_pc_subfields_not_numeric_octets() -> None:
    with pytest.raises(DaoError, match="no active Path Control"):
        DaoManager(node_address=ROOT, is_root=True, pcs=0).process_dao(
            make_dao(N1, ROOT, control=0x40)
        )

    root = DaoManager(node_address=ROOT, is_root=True, pcs=1)
    root.process_dao(make_dao(N1, ROOT))
    root.process_dao(make_dao(N2, ROOT))
    root.process_dao(
        grouped_dao(
            ...
            [
                TransitInformation(N2, path_sequence=1, path_control=0x80),
                TransitInformation(N1, path_sequence=1, path_control=0x40),
            ],
        )
    )
    assert root.routing_table.lookup(N3) == [N1, N3]
...
def test_production_harness_applies_canonical_literal_dao_vectors() -> None:
    vector_path = Path(__file__).parents[3] / "test" / "vectors" / "rpl_route_state.json"
    manager = run_route_state_vectors(vector_path)
    expected_sequences = {
        IPv6Address("2001:db8:3::3"): 2,
        IPv6Address("2001:db8:5::5"): 2,
        IPv6Address("fd00::2"): 1,
        IPv6Address("fd00::3"): 1,
        IPv6Address("fd00::4"): 1,
        IPv6Address("fd00::5"): 1,
        IPv6Address("fd00::6"): 1,
    }
    assert manager._path_sequences == expected_sequences


def test_literal_vector_bytes_select_exact_profile_path() -> None:
    import json

    vector_path = Path(__file__).parents[3] / "test" / "vectors" / "rpl_route_state.json"
    document = json.loads(vector_path.read_text())
    oracle = document["oracle"]
    root_address = IPv6Address(bytes.fromhex(oracle["dodag_id"]))
    parent_two = IPv6Address("fd00::2")
    parent_three = IPv6Address("fd00::3")
    target = IPv6Address("2001:db8:1::1")
    root = DaoManager(
        node_address=root_address,
        is_root=True,
        dodag_id=root_address,
        lifetime_unit_seconds=oracle["lifetime_unit_seconds"],
        max_targets=oracle["limits"]["max_targets"],
        max_candidates=oracle["limits"]["max_candidates"],
        max_candidates_per_target=oracle["limits"].get(
            "max_candidates_per_target", oracle["limits"]["max_candidates"]
        ),
    )
    for vector in document["vectors"]:
        root.process_dao_at(
            DAO.from_bytes(bytes.fromhex(vector["dao_hex"])),
            vector["now_seconds"],
        )
        if vector["name"] == "initial_multi_parent_grouped_install":
            break
    assert root.routing_table.lookup(target) == [parent_three, target]
    assert root.routing_table.lookup(parent_two) == [parent_two]
